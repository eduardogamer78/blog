<x-layouts.marketing>
  ...
  <article class="relative w-full h-auto mx-auto prose-m prose md:prose-2xl dark:prose-invert">
    <div class="py-6 mx-auto heading md:py-12 lg:w-full md:text-center">
      ...
      @if ($post->image)
          <img src="@if(str_starts_with($post->image, 'https') || str_starts_with($post->image, 'http')){{ $post->iamge }}@else{{ asset('storage/' . $post->image) }}@endif" alt="{{ $post->title }}" class="w-full mx-auto mt-4">
      @endif
